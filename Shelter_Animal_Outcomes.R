...
library(rpart)
am=rpart(AgeDays ~ AnimalType + sex + Fertility + SimpleBreed + Name, data = train[!is.na(train$AgeDays), ], method = 'anova')
train$AgeDays[is.na(train$AgeDays)] =predict(am, train[is.na(train$AgeDays), ])
######################################################################################
#針對訓練資料集進行年齡分類
#找出資料集動物標視為狗
dog=which(train$AnimalType=="Dog")
#抓出動物是狗的資料表
train_dog=train[dog,]
#抓出動物是貓的資料表
train_cat=train[-dog,]
#把狗區分小於2歲的kid,成犬為大於2歲小於7歲，老犬為大於7歲
train_dog$Age=ifelse(train_dog$AgeDays<730,'Kid',ifelse(train_dog$AgeDays>=730 & train_dog$AgeDays<=2555,'Adult','Old'))
#把貓區分小於1歲的kid,成貓為大於1歲小於7歲，老貓為大於7歲
train_cat$Age=ifelse(train_cat$AgeDays<365,'Kid',ifelse(train_cat$AgeDays>=365 & train_cat$AgeDays<=2535,'Adult','Old'))
#重新組合資料集
train=bind_rows(train_dog, train_cat)
#把形容動物年齡的欄位向量化
train$Age=factor(train$Age,levels = c("Kid","Adult","Old"))
#填補 sex 的缺漏值，因為不確定，全部填成Unknown
train$sex[is.na(train$sex)]="Unknown"
#填補 Fertility的缺漏值，因為不確定，全部填成Unknown
train$Fertility[is.na(train$Fertility)]="Unknown"
